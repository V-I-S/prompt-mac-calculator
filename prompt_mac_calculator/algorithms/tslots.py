...
def communication_success_probability(transmitters: int, slots: int, selects: int) -> float:
    ...
    logging.debug(f'Done calculations. slots:{slots}, transmitters:{transmitters}, selects:{selects}, probability:{probability}')
    return probability
...
def _count_combinations_for_transmission_sequence(sequence: Tuple[int], transmitters: int, slots: int, selects: int) -> int:
    """
    ...
    return binomial(slots, sum_seq) \
           * _seqence_cardinality(sequence) \
           * multinomial(sum_seq, sequence) \
           * reduce(operator.mul, map(collided_slots_combinations_func, sequence)) \
           * (-1) ** (transmitters + sum_seq)


def _seqence_cardinality(sequence: Tuple[int]) -> int:
    numerator = factorial(len(sequence))
    denominator = _seqence_cardinality_denominator(sequence)
    assert numerator % denominator == 0
    return numerator // denominator


def _seqence_cardinality_denominator(sequence: Tuple[int]) -> int:
    denominator = 1
    rep = 1
